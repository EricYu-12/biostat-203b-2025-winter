...
**Solution:**
Step 1: Connect to labevents table
```{r}
...
Step 2: Connect d_labitems table and filter it
```{r}
# Define target_items
...
d_labitems_tble
```
Step 3: Filter labevents table with the target items and create the labevents_tble
```{r}
labevents_tble <- labevents_data |>
  ...
  # Filter to include only items of interest
  inner_join(select(d_labitems_tble, itemid), by = "itemid") |>
  
  # Join ICU stays table
  left_join(
  ...
  # Remove old itemid columns (optional, if you want to keep only renamed columns)
  select(-`50882`, -`50902`, -`50912`, -`50931`, -`50971`, -`50983`, -`51221`, -`51301`) |>
