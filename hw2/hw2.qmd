...
time_arrow <- system.time({
  # Open the CSV with Arrow, specify format explicitly
  dataset <- open_dataset("~/mimic/hosp/labevents.csv", format = "csv")
  
  # Process data in batches
  result_tibble <- dataset %>%
    select(subject_id, itemid, value, flag) %>%
    filter(itemid %in% c(50868, 50902, 50912)) %>%
    collect()  # Collect only filtered data into memory
})
