...
print(results)
```
As shown above, fread in the data.table package is the fastest while read.csv in base R is the slowest, and there is difference in the default parsed data types. The memory taken up each resultant dataframe or tibble use are also shown above.
...
time_with_col_types <- system.time({
  df_col_types <- read_csv(file_path, col_types = col_types_specified)
})
...
```
The run time changes. The run time and the memory used by the result tibble are shown above.
...
```
My system did not finish the process of ingestion in 3 minutes because my laptop does not have enough memory (RAM) to ingest the large labevents.cvs file.
...
```
My program is forced to terminate again since my laptop still does not have enough RAM to ingest the data even just trying to ingest some specific columns.
...
**Solution:**
First, unzip the labevents.csv.gz file
```{bash}
#| eval: false
gunzip -k ~/mimic/hosp/labevents.csv.gz
```
Measure the time for ingest + select + filter
```{r}
...
```
Lastly, show the first 10 rows:
```{r}
# Display the first 10 rows
print(head(result_tibble, 10))
```
Apache Arrow is a method to move and process large amounts of data within a short time. Imagine you bought a large bag of rice from Costco and trying to pour it into a bucket. For most of the time, if you are not strong enough to carry the bag, you would use a small rice cup to shovel the rice cup by cup to the bucket, which could take a long time. Apache Arrow is the method that lets you move the entire bag of rice at once and pour it into the rice bucket, which is more time efficient. It stores data in a format that different tools and programs can understand instantly, making data analysis much faster, especially when dealing with big files that don’t even fit into your computer’s memory.
...
**Solution:**
First, re-write the csv file into binary Parquet format:
```{r}
# Define the CSV file path and the output Parquet directory
...
```
Then, measure the size of the file:
```{bash}
du -sh labevents_parquet
```
After that, measure the time for ingest + select + filter
```{r}
...
print(time_parquet)
```
Not stopping here, we are going to display the number of rows:
```{r}
cat("Number of Rows:", nrow(result_tibble_parquet), "\n")
```
Finally, display the first 10 rows:
```{r}
print(head(result_tibble_parquet, 10))
```
Parquet is a method that allow people to efficiently store large datasets. It functions like a higly organized, compressed data package which allow users to process the data within a short time. It’s a useful tool useful when dealing with large amount of data since it saves space and speeds up data processing.
...
**Solution:**
First, ingest the Parquet file and convert it to a DuckDB table, and measure the time it takes to finish the process:
```{r}
