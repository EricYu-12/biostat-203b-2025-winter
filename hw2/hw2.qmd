...
```{r}
# Measure time for ingest + select + filter
time_arrow <- system.time({
  dataset <- open_dataset("~/mimic/hosp/labevents.csv", format = "csv")
...
**Solution:**
Re-write the csv file into binary Parquet format
```{r}
# Define the CSV file path and the output Parquet directory
labevents_data <- open_dataset("~/mimic/hosp/labevents.csv", format = "csv")

# Write to Parquet format
write_dataset(labevents_data, path = "labevents_parquet", format = "parquet")
```
Measure the size of the file:
```{bash}
du -sh labevents_parquet
```
Measure the time for ingest, select, and filter
```{r}
# Measure the time for ingest, select, and filter
time_parquet <- system.time({
  dataset_parquet <- open_dataset("labevents_parquet", format = "parquet")

# Select specific columns and apply filter
