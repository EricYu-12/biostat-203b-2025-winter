...
**Solution:** The size of the compressed and uncompressed file respectively:
```{bash}
...
```
Regarding trade offs between storage and speed for big data files, it is obvious that the compressed file takes up significantly less storage space than the uncompressed file, and the compressed file has significantly shorter run times than the uncompressed file due to smaller file size. In general, for large datasets, the compressed file takes up smaller storage space and shorter run time than the uncompressed file.
